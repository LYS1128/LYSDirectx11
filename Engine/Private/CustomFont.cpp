...
HRESULT CCustomFont::Render(const _tchar* pText, _float2 vPosition, _fvector vColor, float rotation, _float2 const& Float2Zero, float scale)
{
	m_pBatch->Begin();


	m_pFont->DrawString(m_pBatch, pText, vPosition, vColor, rotation, Float2Zero, scale);
