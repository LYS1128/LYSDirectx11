...
HRESULT CFont_Manager::Render_Fonts(const _tchar * pFontTag, const _tchar * pText, _float2 vPosition, _fvector vColor, float rotation, _float2 const& Float2Zero, float scale)
{
	CCustomFont*		pFont = Find_Fonts(pFontTag);
	if (nullptr == pFont)
		return E_FAIL;

	return pFont->Render(pText, vPosition, vColor,  rotation,  Float2Zero,scale);
}
