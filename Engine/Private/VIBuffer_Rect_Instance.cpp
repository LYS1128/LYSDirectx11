...
#pragma region INSTANCE_BUFFER
	...
	for (_uint i = 0; i < m_iNumInstance; ++i)
	{
		...
		_vector	vRight = XMVector3Cross(XMVectorSet(0.f, 0.f, 1.f, 0.f), vLook);
		_vector	vUp = XMVector3Cross(vLook, vRight);

		vRight *= 0.1*(rand() % 2);

		_float4 Look, Right, Up;
		XMStoreFloat4(&Look, vLook);
		XMStoreFloat4(&Right, vRight);
		XMStoreFloat4(&Up, vUp);
		
		pInstanceVertices[i].vLook = Look;
		pInstanceVertices[i].vRight = Right;
		pInstanceVertices[i].vUp = Up;
...
HRESULT CVIBuffer_Rect_Instance::Update(_double TimeDelta)
{
	...
	for (_uint i = 0; i < m_iNumInstance; ++i)
	{
		...
		vPosition += vUp * m_pInstanceSpeed[i] * TimeDelta;
