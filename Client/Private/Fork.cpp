...
HRESULT CFork::NativeConstruct(void * pArg)
{
	...
	if (FAILED(pGameInstance->Initialize_Navigation(m_pDevice, m_pDeviceContext, TEXT("../../Data/Gara17.dat"), m_pTransformCom)))
			return E_FAIL;

	for (_uint i = 0; i < 6; ++i) {
		if (FAILED(pGameInstance->Set_CellOption(535+i,2)))
			return E_FAIL;
	}
...
void CFork::LateTick(_double TimeDelta)
{
	...
	if (nullptr != m_pRendererCom &&  Index <=396)
...
HRESULT CFork::Render_LightDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
