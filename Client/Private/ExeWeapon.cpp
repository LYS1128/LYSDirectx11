#include "stdafx.h"
#include "..\Public\ExeWeapon.h"
#include "GameInstance.h"

CExeWeapon::CExeWeapon(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CGameObject(pDeviceOut, pDeviceContextOut)
{

}

CExeWeapon::CExeWeapon(const CExeWeapon & rhs)
	: CGameObject(rhs)
{
}

HRESULT CExeWeapon::NativeConstruct_Prototype()
{
...
HRESULT CExeWeapon::NativeConstruct(void * pArg)
{
	if (nullptr != pArg)
	...
	m_pPlayer = (CCardGuard*)(pGameInstance->Find_Layer_List(LEVEL_BOSS, L"Layer_Executioner")->front());
	m_pPlayerTrans = (CTransform*)(pGameInstance->Get_Component(LEVEL_BOSS, L"Layer_Executioner", L"Com_Transform"));
...
void CExeWeapon::Tick(_double TimeDelta)
{
...
void CExeWeapon::LateTick(_double TimeDelta)
{
	...
	if (pGameInstance->Find_Layer_List(LEVEL_BOSS, L"Layer_Executioner")->size() == 0)
		m_eState = STATE_DEAD;
...
HRESULT CExeWeapon::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::SetUp_Components(TEXT("Com_Model"), LEVEL_BOSS, TEXT("Prototype_Component_Model_ExeWeapon"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CExeWeapon::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CExeWeapon * CExeWeapon::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CExeWeapon*	pInstance = new CExeWeapon(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CExeWeapon"));
...
CGameObject * CExeWeapon::Clone(void * pArg)
{
	CExeWeapon*	pInstance = new CExeWeapon(*this);
