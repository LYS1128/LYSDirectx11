#include "stdafx.h"
#include "..\Public\FleshLamp.h"
#include "GameInstance.h"

CFleshLamp::CFleshLamp(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CGameObject(pDeviceOut, pDeviceContextOut)
{

}

CFleshLamp::CFleshLamp(const CFleshLamp & rhs)
	: CGameObject(rhs)
{
}

HRESULT CFleshLamp::NativeConstruct_Prototype()
{
...
HRESULT CFleshLamp::NativeConstruct(void * pArg)
{
	...
	m_pTransformCom->Rotation(XMVectorSet(0.f, 0.f, 1.f, 0.f), XMConvertToRadians(-45.0f));
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(-130.0f, -22.f, 163.0f, 1.f));
	m_pAABBCom->Set_Active(true);
	return S_OK;
}

void CFleshLamp::Tick(_double TimeDelta)
{
	__super::Tick(TimeDelta);
	if (XMVectorGetY(m_pTransformCom->Get_State(CTransform::STATE_POSITION)) >= -27.f) {
		_vector fGo = XMVector4Normalize(m_pTransformCom->Get_State(CTransform::STATE_POSITION) - XMVectorSet(-135.0f, -27.f, 163.0f, 1.f));
		m_pTransformCom->Set_State(CTransform::STATE_POSITION, m_pTransformCom->Get_State(CTransform::STATE_POSITION) - 1.8f*fGo);
	}
	else if (m_iSpawn == false) {
		m_iSpawn = true;
		CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
		pGameInstance->StopSound(CSound_Manager::SCENE_SOUND2);
		pGameInstance->SoundPlay(L"CardGuard_WeaponDrop.ogg", CSound_Manager::SCENE_SOUND2, SOUND_DEFAULT);
...
void CFleshLamp::LateTick(_double TimeDelta)
{
...
HRESULT CFleshLamp::Render_LightDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CFleshLamp::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	///* For.Com_AABB */
	ColliderDesc.vPosition = _float3(0.f, 3.7f, 0.f);
	ColliderDesc.vSize = _float3(2.2f, 10.f, 2.2f);
...
HRESULT CFleshLamp::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CFleshLamp * CFleshLamp::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CFleshLamp*	pInstance = new CFleshLamp(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CFleshLamp"));
...
CGameObject * CFleshLamp::Clone(void * pArg)
{
	CFleshLamp*	pInstance = new CFleshLamp(*this);
