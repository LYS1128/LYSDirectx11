#include "stdafx.h"
#include "..\Public\ForkThree.h"
#include "GameInstance.h"

CForkThree::CForkThree(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CGameObject(pDeviceOut, pDeviceContextOut)
{

}

CForkThree::CForkThree(const CForkThree & rhs)
	: CGameObject(rhs)
{
}

HRESULT CForkThree::NativeConstruct_Prototype()
{
...
HRESULT CForkThree::NativeConstruct(void * pArg)
{
...
void CForkThree::Tick(_double TimeDelta)
{
	__super::Tick(TimeDelta);

}

void CForkThree::LateTick(_double TimeDelta)
{
	...
	if (nullptr != m_pRendererCom &&  Index >= 390)
...
HRESULT CForkThree::Render_LightDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CForkThree::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::SetUp_Components(TEXT("Com_Model"), LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_ForkLift3"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
	
	return S_OK;
}

HRESULT CForkThree::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CForkThree * CForkThree::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CForkThree*	pInstance = new CForkThree(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CForkThree"));
...
CGameObject * CForkThree::Clone(void * pArg)
{
	CForkThree*	pInstance = new CForkThree(*this);
