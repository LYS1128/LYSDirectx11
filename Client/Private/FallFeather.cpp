#include "stdafx.h"
#include "..\Public\FallFeather.h"
#include "GameInstance.h"

CFallFeather::CFallFeather(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CGameObject(pDeviceOut, pDeviceContextOut)
{

}

CFallFeather::CFallFeather(const CFallFeather & rhs)
	: CGameObject(rhs)
{
}

HRESULT CFallFeather::NativeConstruct_Prototype()
{
...
HRESULT CFallFeather::NativeConstruct(void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION,Pos + XMVectorSet(((rand() % 21) - 10) / 10.f, 0.f, ((rand() % 21) - 10) / 10.f, 0.f));
...
void CFallFeather::Tick(_double TimeDelta)
{
...
void CFallFeather::LateTick(_double TimeDelta)
{
...
HRESULT CFallFeather::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (Rand == 0) {
		if (FAILED(__super::SetUp_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Prototype_Component_Model_JumpFeatherR"), (CComponent**)&m_pModelCom)))
			return E_FAIL;
	}
	else if (Rand == 1){
		if (FAILED(__super::SetUp_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Prototype_Component_Model_JumpFeatherG"), (CComponent**)&m_pModelCom)))
			return E_FAIL;
	}
	else {
		if (FAILED(__super::SetUp_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Prototype_Component_Model_JumpFeatherB"), (CComponent**)&m_pModelCom)))
			return E_FAIL;
	}
...
HRESULT CFallFeather::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CFallFeather * CFallFeather::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CFallFeather*	pInstance = new CFallFeather(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CFallFeather"));
...
CGameObject * CFallFeather::Clone(void * pArg)
{
	CFallFeather*	pInstance = new CFallFeather(*this);
