#include "stdafx.h"
#include "..\Public\DollHead.h"
#include "GameInstance.h"

CDollHead::CDollHead(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CGameObject(pDeviceOut, pDeviceContextOut)
{

}

CDollHead::CDollHead(const CDollHead & rhs)
	: CGameObject(rhs)
{
}

HRESULT CDollHead::NativeConstruct_Prototype()
{
...
HRESULT CDollHead::NativeConstruct(void * pArg)
{
...
void CDollHead::Tick(_double TimeDelta)
{
...
void CDollHead::LateTick(_double TimeDelta)
{
...
HRESULT CDollHead::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model */
	if (FAILED(__super::SetUp_Components(TEXT("Com_Model"), LEVEL_BOSS, TEXT("Prototype_Component_Model_DollHead"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CDollHead::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CDollHead * CDollHead::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CDollHead*	pInstance = new CDollHead(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CDollHead"));
...
CGameObject * CDollHead::Clone(void * pArg)
{
	CDollHead*	pInstance = new CDollHead(*this);
