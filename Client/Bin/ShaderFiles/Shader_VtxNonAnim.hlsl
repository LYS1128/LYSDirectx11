...
VS_OUT_LIGHTDEPTH VS_MAIN_MODEL_LIGHTDEPTH(VS_IN In)
{
...
PS_OUT PS_MAIN_WATER(PS_IN_MODEL In)
{
	PS_OUT			Out = (PS_OUT)0;
	...
	float		DetailLevel = 10.f;
	float2		NewUV = In.vTexUV*DetailLevel;
	NewUV.y += g_Time;
	vector		vWave = g_WaveTexture.Sample(DefaultSampler, NewUV);

	Out.vDiffuse = saturate((Out.vDiffuse*0.5f) + (vWave*0.3f));
...
void PS_MAIN_MODEL_LIGHTDEPTH(PS_IN_LIGHTDEPTH In)
{
	vector		vMtrlDiffuse = g_DiffuseTexture.Sample(DefaultSampler, In.vTexUV);
	if (vMtrlDiffuse.a < 0.1f)
		discard;
...
{	
	...
	pass Model_LightDepth
	{
		SetRasterizerState(RS_LightDepth);

		VertexShader = compile vs_5_0 VS_MAIN_MODEL_LIGHTDEPTH();
		GeometryShader = NULL;
		PixelShader = compile ps_5_0 PS_MAIN_MODEL_LIGHTDEPTH();
	}
	pass Water
	{
		SetRasterizerState(RS_Not_Cull);
